...
The bayesian approach conceptualizes probabilities as uncertainty. This is useful to online learning scenarios, where part of the dataset is encountered during runtime. In order to apply online learning to @exm-anomaly-detection, it's necessary to assume a probability distribution for both the prior and posterior. An useful distribution to model boolean outcomes is the Bernoulli Distribution. In that case, we have:
$$
F\sim Ber(\theta) 
$$

Where $\sim$ implies that the random variable $F$ follows the probability distribution $Ber(\theta) and $\theta\in[0,1]$ represents how probable that $F=f$. We can rewrite the likelihood differently:
