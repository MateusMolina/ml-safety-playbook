...
$$
p(\theta_d \vert \ \mathcal{D}_f, f)=?
$$

Where $\theta_d$ is the probability of the part being defective, $\mathcal{D}_f=\{D_1, D_2, \dots, D_i\}$ is the set of observations of $D$ outcomes when $F=f$. Interestingly, $\theta_d$ is both a probability and a random variable itself, since we are uncertain about its value.

We can once again express our task in terms of the bayesian equation (@eq-bayes):
$$
\begin{align*}
p(\theta_d \vert \ \mathcal{D}_f, f)&\propto p(\mathcal{D}_f, f\vert \theta_d)p(\theta_d) \\ 
&\propto p(\mathcal{D}_f\vert \theta_d, f)p(f\vert \theta_d)p(\theta_d) \\
&\propto p(\mathcal{D}_f\vert \theta_d, f)p(\theta_d)\\
\end{align*}
$$ {#eq-exm-update}

Since $f$ is an observation and thus not directly governed by $\theta_d$ -- it's independent to it, we can drop the term $p(f\vert \theta_d)$, as it will be a constant factor in the expression and it will be absorbed by normalization.
...
In our case, we can use the Beta distribution as our prior and the Bernoulli distribution as our likelihood. Specifically, we can model the prior distribution of $\theta_d$ as a Beta distribution, $Beta(\alpha_d, \beta_d)$, and the likelihood of the observed data as a Bernoulli distribution, $Ber(\theta_d)$.

$$
\mathcal{D}_f\vert \theta_d, f \sim Ber(\theta_d)
$$
$$
\theta_d \sim Beta(a, b)
$$
...
$$p(\mathcal{D}_f\vert \theta_d, f)=\theta_d^{N_{f,d}}(1-\theta_d)^{N_{f,nd}}$$
$$p(\theta_d)\propto \theta_d^{a-1}(1-\theta_d)^{b-1}$$
...
$$
\begin{align*}
p(\theta_d \vert \ \mathcal{D}_f, f)&\propto Ber(\theta_d)Beta(a, b) \\
&\propto \theta_d^{N_{f,d}}(1-\theta_d)^{N_{f,nd}}\theta_d^{a-1}(1-\theta_d)^{b-1} \\
&\propto \theta_d^{N_{f,d}+a-1}(1-\theta_d)^{N_{f,nd}+b-1} \\
...
$$
\begin{align*}
\hat{\theta_d}\vert \ \mathcal{D}_f, f&=\frac{a+N_{f,d}-1}{a+b+N_{f,d}+N_{f,nd}-2} \\
...
plt.xlabel('Probability of Defective Part (p)')
...
To see how new observations gradually nudge our posterior beliefs, the next simulation follows 1000 consecutive parts. After each part, the posterior is updated and the mode and the error bar is computed based on @eq-beta-props. The error bar is computed as one standard deviation around the mode, assuming a latent true latent defect probability of flagged events of $0.6$.
...
for t in range(T):
    ...
    # Mode (MAP) for Beta when a,b > 1: (a-1)/(a+b-2)
    if a > 1 and b > 1:
        mode = (a - 1) / (a + b - 2)
    else:
        # mode undefined at edges; fallback to mean
        mode = a / (a + b)
...
plt.fill_between(x, modes - stds, modes + stds, color='green', alpha=0.2, label='±1 std')
plt.hlines(y=true_p, xmin=1, xmax=T, color='red', linestyle='--', label=f'True p={true_p:.3f}')
plt.title('Online Beta-Bernoulli Updates (MAP and ±1 std)')
plt.xlabel('Observation (t)')
plt.ylabel('Estimated defect probability')
plt.ylim(0.4, 0.8)
plt.legend()
plt.grid(alpha=0.3)
plt.tight_layout()
...
1. Add insights about the plot
2. Simplify $\theta_d\vert f$ to $\theta_{d,f}$ since we are computting the posterior only when $F=f$
3. The error at the beginning is too small, how can we improve this?
4. Add algorithm for the online update