...
To see how new observations gradually nudge our posterior beliefs, the next simulation follows 1000 consecutive parts. After each part, the posterior is updated and the mode and the error bar is computed based on @eq-beta-props. The error bar is computed as three times the standard deviation ($3\sigma$) around the mode, assuming a latent true latent defect probability of flagged events of $\theta_{f,d}^\star=0.6$.
