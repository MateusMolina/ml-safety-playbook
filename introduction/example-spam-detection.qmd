...
where $\forall \theta_{jc} = p(x_{ij}=1\vert y_i=c) \in \pmb{\Theta}$ is the probability of word $j$ appearing in messages of class $c$.

**Modelling the y vector**. To complete @eq-exm-spam-task, we also need to model the prior $p(y_i=c\vert \pmb{\Theta})$. Since for each message $i$ we only have one class $y_i$, we can model it using a categorical distribution:
$$y_i \vert \pmb{\pi} \sim cat(y_i\vert \pmb{\pi})$$ 
where $\pmb{\pi} = \{\pi_c\}_{c \in \mathcal{C}} \in \pmb{\Theta}$ are the prior probabilities for each class in $\mathcal{C}$.
...
By using the assumption in @eq-exm-spam-likelihood, we can expand the term $p(\mathbf{x}_i\vert y_i, \pmb{\theta})$ in @eq-exm-spam-training-likelihood-2 as follows:
$$
...
By using @prp-categorical-indicators, we can rewrite the equation above as:
...
$$ {#eq-exm-spam-training-likelihood-4} 

Returning to @eq-exm-spam-training-likelihood, assuming independence between data points, we can express the full likelihood of the dataset $\mathcal{D}$ as:

$$
p(\mathcal{D}\vert \pmb{\Theta}) = \prod_{i} p(\mathbf{x}_i,y_i\vert \pmb{\Theta})
$$ {#eq-exm-spam-training-likelihood-5}

Plugging @eq-exm-spam-training-likelihood-4 into @eq-exm-spam-training-likelihood-5, and taking the logarithm to simplify the product into a sum, we get:
$$
